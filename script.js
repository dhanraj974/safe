...
function triggerEmergencyAlert(triggerMethod) {
    ...
    // Get current location (works even offline - GPS doesn't need internet)
...
function showAlertModal(contacts, alertData) {
    ...
    if (contacts.length === 0) {
        ...
        const fullMessage = encodeURIComponent(alertData.message + 
            (alertData.location ? ` Location: https://www.google.com/maps?q=${alertData.location.latitude},${alertData.location.longitude}` : ''));
