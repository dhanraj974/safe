...
function retryLocation() {
    ...
    getCurrentLocationWithRetry().then(() => {
        ...
        // Also show manual message option
        showManualMessage(fullMessage, contacts);
        ...
        smsButtonsContainer.innerHTML = `
            ...
            <p style="font-size: 0.8rem; color: var(--secondary-text); margin-top: 8px;">If SMS doesn't work, use the manual message below</p>
        `;
...
function showManualMessage(message, contacts) {
    const manualContainer = document.getElementById('manualMessageContainer');
    const manualText = document.getElementById('manualMessageText');
    
    if (manualContainer && manualText) {
        manualContainer.style.display = 'block';
        
        // Add contact info to message
        let fullMessage = message;
        if (contacts.length > 0) {
            const contactList = contacts.map(c => c.phone).join(', ');
            fullMessage = `Send to: ${contactList}\n\n${message}`;
        }
        
        manualText.value = fullMessage;
    }
}
...
function showAlertModal(contacts, alertData) {
    ...
    if (contacts.length === 0) {
        ...
        // Show manual message option (always shown as fallback)
        showManualMessage(fullMessage, contacts);
